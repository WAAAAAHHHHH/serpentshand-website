...
export async function onRequestPost(context) {
  try {
    ...
    const info = await db.prepare(
      "INSERT INTO messages (name, subject, entry, replies, created_at) VALUES (?, ?, ?, ?, ?)"
    ).bind(
      name.substring(0, 40), 
    ...
    return new Response(JSON.stringify({ error: "Failed to create thread. Please try again later." }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
  }
