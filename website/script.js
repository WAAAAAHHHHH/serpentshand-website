(() => {
  ...
  const ANNOUNCEMENTS = [
    ...
    'The Reading Room is open below. Entries are visible only in your own browser.',
  ...
  (() => {
    ...
    const STORAGE_KEY = 'archive_forum_threads_v1';
    ...
    const loadThreads = () => {
      ...
      // Seed content shown on first visit
      return [
        {
          id: 'seed-1',
          name: 'The Keeper',
          subject: 'Welcome to the Reading Room',
          body: 'Leave an entry below if you found something worth recording. Nothing here is moderated, and nothing leaves this browser — treat it like a guestbook rather than a public wall.',
          time: Date.now() - 1000 * 60 * 60 * 24 * 3,
          replies: [
            { name: 'A Visitor', body: 'Good to know it stays local. Still nice to have a place to leave a note.', time: Date.now() - 1000 * 60 * 60 * 20 },
          ],
        },
      ];
    };
    ...
    const render = () => {
      ...
      sorted.forEach((thread) => {
        ...
        replyForm.addEventListener('submit', (e) => {
          ...
          target.replies.push({ name, body: replyBody, time: Date.now() });
          saveThreads(threads);
          render();
        });
